...
class Dimensions {
  constructor (viewQuery) {
    ...
    const projectedColumns = [...view.out(ns.view.projection).out(ns.view.columns).list?.() || []]

    this.array = distinct(resultDimensions, filterDimensions).toArray().map(dimension => {
      const property = resolveProperty(dimension)
      const isResult = projectedColumns.some(column => column.term.equals(dimension.term))

...
class Dimensions {
  constructor (viewQuery) {
    ...
    this.resolveProperty = function (dimensionPtr) {
      if (dimensionPtr.out(ns.view.as).term) {
        return dimensionPtr.out(ns.view.as).term
      } else if (dimensionPtr.out(ns.view.from).out(ns.view.path).term) {
        return dimensionPtr.out(ns.view.from).out(ns.view.path).term
      } else {
    ...
    this.array = distinct(resultDimensions, filterDimensions).toArray().map(dimension => {
      const property = this.resolveProperty(dimension)

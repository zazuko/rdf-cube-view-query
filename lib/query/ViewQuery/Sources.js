...
import { contains, distinct } from '../utils.js'
...
export default class Sources {
  constructor(viewQuery) {
    ...
    const sources = distinct(view.node(dimensions.map(d => d.ptr.term)).out(ns.view.from).out(ns.view.source))

    this.array = sources.toArray().map(source => {
      const graph = source.out(ns.view.graph).term
      const isCubeSource = contains(source.out(ns.rdf.type), ns.view.CubeSource)
      const isLookupSource = contains(source.out(ns.rdf.type), ns.view.LookupSource)
  ...
  initPattern({ source }) {
    ...
    if (source.isCubeSource) {
      ...
      source.patterns.push([cube, ns.cube.observationSet, source.variableSet, graph])
      source.patterns.push([source.variableSet, ns.cube.observation, source.variable, graph])
    }
  }

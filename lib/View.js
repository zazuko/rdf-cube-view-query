...
`
    ...
    return `${this.queryPrefix}DESCRIBE ?s FROM <${this.source.graph.value}>
WHERE {
    <${cubeUri}> <https://cube.link/observationConstraint> ?s
}
`
  ...
  async fetchCubeShape () {
    ...
    const shapeData = await this.source.client.query.construct(this.cubeShapeQuery(cube.value))

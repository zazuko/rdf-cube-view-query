...
class View extends Node {
  ...
  static fromCube (cube) {
    ...
    cube.dimensions.forEach(cubeDimension => {
      ...
      orderingColumns.push(viewDimension)
      view.addDimension(viewDimension)
    })

    const sortAlphaNum = (a, b) => a.cubeDimensions[0].path.value.localeCompare(b.cubeDimensions[0].path.value, 'en', { numeric: true })
    orderingColumns.sort(sortAlphaNum)

    const projectionPtr = view.ptr.out(ns.view.projection)
    projectionPtr.addList(ns.view.columns, orderingColumns.map(dimension => dimension.ptr.term))

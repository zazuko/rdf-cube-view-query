...
`
  ...
  fetchViewQuery () {
    ...
    const query = viewDefQuery({ view: this.term, graph: this.getOneSource().graph })
  ...
  async fetchView () {
    ...
    const viewData = await this.getOneSource().client.query.construct(query)
  ...
  fetchViewFullQuery () {
    const prefix = this.queryPrefix || ''
    const query = viewFullQuery({ view: this.term, graph: this.graph })
    return [prefix, query.toString()].join('')
  }

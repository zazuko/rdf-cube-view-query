...
class View extends Node {
  ...
  async observations ({ disableDistinct } = {}) {
    ...
    const source = this.getOneSource()
  ...
  cubeShapeQuery (cubeUri) {
    if (!this.getOneSource().graph) {
      return `${this.queryPrefix}DESCRIBE ?s
WHERE {
...
`
    ...
    return `${this.queryPrefix}DESCRIBE ?s FROM <${this.getOneSource().graph.value}>
WHERE {
    <${cubeUri}> <https://cube.link/observationConstraint> ?s
}
`
  ...
  async fetchCubeShape () {
    // const cube = this.ptr.node(this.dimensions[0].term).out(ns.view.from).out(ns.view.source).out(ns.view.cube).term
    const cube = this.cubes()[0]
    if (!cube) {
      throw new Error('No cube found')
    }
    const shapeData = await this.getOneSource().client.query.construct(this.cubeShapeQuery(cube.value))
    if (!shapeData.length) {
      throw Error('No shape data found')
    }
  ...
  async init (source) {
    ...
    await this.fetchViewFull(source)
    this.updateEntities(source)
  }

  fetchViewFullQuery (source) {
    const prefix = this.queryPrefix || ''
    const query = viewFullQuery({ view: this.term, graph: source.graph })
    return [prefix, query.toString()].join('')
  }

  async fetchViewFull (source) {
    const query = this.fetchViewFullQuery(source)
  ...
  getOneSource () {
    return this.sources().length ? this.sources()[0] : undefined
  }
  ...
  cubes () {
    const arr = this.dimensions.filter(x => x.cube).map(x => x.cube)
    return [...new Map(arr.map((item, key) => [item[key], item])).values()]
  }
}

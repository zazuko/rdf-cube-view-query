...
describe('.resolveProperty', () => {
  it('should be a method', () => {
    const view = clownface({ dataset: rdf.dataset(), term: ns.ex.view })
    const dimensions = new Dimensions({ view, variable: () => {} })

    strictEqual(typeof dimensions.resolveProperty, 'function')
  })
...
`
  ...
  it('gets the property from path when view:as is not defined', () => {
    const viewTTL = `
@prefix view: <https://cube.link/view/> .
@prefix ex: <http://example.org/> .
ex:view a view:View ;
...
`
    ...
    strictEqual('BlankNode', dimensions.array[0].property.constructor.name)
  })
})
